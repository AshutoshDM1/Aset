...
export function FontInitializer() {
  useEffect(() => {
    try {
      const savedFont = localStorage.getItem('aset-selected-font');
      const isGlobal =
        localStorage.getItem('aset-global-font-enabled') === 'true';
...
export default function FontChanger() {
  ...
  const [customText, setCustomText] = useState(
    'Aset Drive: Secure, cloud-native file storage & management.',
  );
  ...
  const handleSave = () => {
    startTransition(() => {
      try {
        localStorage.setItem('aset-selected-font', selectedFont);
        localStorage.setItem(
          'aset-global-font-enabled',
  ...
  const handleReset = () => {
    startTransition(() => {
      try {
        localStorage.removeItem('aset-selected-font');
        localStorage.removeItem('aset-global-font-enabled');
