...
export const statsRouter = router({
  getDashboardStats: protectedProcedure
    .input(
      z
        .object({
          rangeDays: z.number().min(1).max(90).default(7),
        })
        .optional(),
    )
    .query(async ({ ctx, input }) => {
      ...
      // 4. Upload and Download activity chart data
      ...
      const startOfPeriod = new Date(lastNDays[0]);
      ...
      const uploadsChart = lastNDays.map((date) => {
        const label =
          rangeDays > 7
